...
class Config:
    ...
    def load(cls, path: str = "config.yaml") -> "Config":
        ...
        return cls(
            ...
            GEMINI=GeminiConfig(
                API_KEYS=data["GEMINI"]["API_KEYS"] if "GEMINI" in data else [],
                MODEL=data["GEMINI"]["MODEL"] if "GEMINI" in data else "gemini-2.0-flash",
                PROXY_FOR_GEMINI=data["GEMINI"]["PROXY_FOR_GEMINI"] if "GEMINI" in data else "",
            ),
        )
